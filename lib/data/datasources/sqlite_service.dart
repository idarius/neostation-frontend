...
''',
  ...
  /// Searches games whose display name contains [query] (case-insensitive
  /// substring on the COALESCE'd `game_display_name`, which already falls back
  /// to the raw `ur.filename` for non-scraped games). Returns at most [limit]
  /// rows, sorted favorites-first then alphabetical (same as [getAllGames]).
  static Future<List<DatabaseGameModel>> searchGames(
    ...
    final results = await db.rawQuery(
      ...
      JOIN app_systems s ON ur.app_system_id = s.id
      LEFT JOIN user_screenscraper_metadata usm ON ur.app_system_id = usm.app_system_id AND ur.filename = usm.filename
      WHERE LOWER(COALESCE(usm.real_name, CASE WHEN s.folder_name IN ('android') THEN ur.title_name END, ur.filename)) LIKE LOWER(?)
      ORDER BY ur.is_favorite DESC, LOWER(game_display_name) ASC
      LIMIT ?
      ''',
      ['%$query%', limit],
