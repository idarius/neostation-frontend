...
class GamepadsAndroidPlugin: FlutterPlugin, MethodCallHandler, ActivityAware {
  ...
  private fun listGamepads(): List<Map<String, Any>>  {
    return devices.getDevices().map { device ->
      val inputDevice = device.value
      mapOf(
        "id" to device.key.toString(),
        "name" to inputDevice.name,
        "systemInfo" to mapOf(
          "vendorId" to inputDevice.vendorId.toString(16).padStart(4, '0'),
          "productId" to inputDevice.productId.toString(16).padStart(4, '0')
        )
      )
    }
  }
