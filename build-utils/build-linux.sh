...
ARCH=x86_64 "$PLUGIN_DIR/usr/bin/appimagetool" "$APPDIR"

# Move to release
if ls "$PROJECT_ROOT/build-utils/appimage/"*.AppImage 1> /dev/null 2>&1; then
    mv "$PROJECT_ROOT/build-utils/appimage/"*.AppImage "$PROJECT_ROOT/release/neostation-linux-x86_64-${VERSION}.AppImage"
    echo ""
